...
MAKEFILE_DIR = Path("../../../src_new/Makefiles")
...
def setup_simulation(module_name: str, template_name: str):
    ...
    include_path = makefile_dir_path / "tb_template_uvm.include"
    target_makefile = module_dir / "Makefile"
    target_include = module_dir / f"tb_{module_name}.include"

    # Link script path
    link_script_src = (cwd / SCRIPTS_DIR / "link_files_uvm.py").resolve()
    link_script_dest = module_dir / "link_files_uvm.py"
    ...
    if link_script_src.exists():
        try:
            if not link_script_dest.exists():
                ...
                print(f"Created symbolic link for link_files_uvm.py -> {link_script_src}")
            else:
                print("link_files_uvm.py already exists in target directory. Skipping symlink.")
        except Exception as e:
            print(f"Error creating symlink for link_files_uvm.py: {e}")
            sys.exit(1)
    else:
...
if __name__ == "__main__":
    ...
    parser.add_argument("-t", "--template", default="Makefile.sim_uvm", 
                        help="The filename of the template Makefile (defaults to 'Makefile.sim_uvm').")
